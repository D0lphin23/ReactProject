...
// Сортировка книг
const bookConatiner = document.querySelector(".books");
const books = Array.from(document.querySelectorAll(".book"));

books.sort((a, b) => {
    const numA = parseInt(
        a.querySelector("a").textContent.trim().split(" ")[1],
    );
    const numB = parseInt(
        b.querySelector("a").textContent.trim().split(" ")[1],
    );
    return numA - numB;
});

books.forEach((book) => bookConatiner.append(book));
...
// Сортировка глав в книгах 2 и 5

const book2 = books[1];
const book5 = books[4];

const chapters2 = Array.from(book2.querySelectorAll("li"));
const chapters5 = Array.from(book5.querySelectorAll("li"));

function sortChapters(chapters) {
    const ul = chapters[0].parentElement;

    chapters.sort((a, b) => {
        function getIndex(li) {
            const text = li.textContent.trim();

            if (text.includes("Введение")) return 0;
            if (text.includes("Предисловие")) return 1;
            if (text.includes("Глава")) {
                return parseInt(text.split(" ")[1]);
            }

            const appendixMatch = text.match(/Приложение ([A-Z])/);
            if (appendixMatch) {
                return 100 + appendixMatch[1].charCodeAt(0) - "A".charCodeAt(0);
            }
        }
    ...
    return chapters.forEach((li) => ul.append(li));
}

sortChapters(chapters2);
sortChapters(chapters5);

// Добавить главу в книге 6

const books6 = books[5];

const chapters6 = Array.from(books6.querySelectorAll("li"));

const cloneElement = chapters6[9].cloneNode(true);

cloneElement.textContent = "Глава 8: За пределами ES6";
chapters6[8].append(cloneElement);
